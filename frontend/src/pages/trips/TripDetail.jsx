...
import {
  ArrowLeft, Calculator, CalendarDays, Check, MapPin, MapPinPlus, Pencil, Share2, Users, Wallet,
} from 'lucide-react'
      ...
      <PageContainer>
        ...
        <header className="mb-8 flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span
                className={cn(
                  ...
                  isSolo ? 'bg-mint-100 text-mint-600 dark:bg-mint-500/15 dark:text-mint-300' : 'bg-lav-100 text-lav-600 dark:bg-lav-500/15 dark:text-lav-300',
                )}
              >
                {isSolo ? '🎒 Solo travelling' : `👥 Group travelling · ${members.length}`}
              </span>
            </div>
            <p className="mt-2 flex flex-wrap items-center gap-x-5 gap-y-1 text-sm text-ink-muted">
              ...
              <span className="flex items-center gap-1.5"><Users className="size-4" aria-hidden />{members.join(' · ')}</span>
            </p>
          ...
          <div className="flex gap-2">
            ...
            <Button variant="secondary" to={`/trips/${trip.id}/budget`}>
        ...
        <section aria-label="Trip totals" className="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
          {[
            ...
            { label: 'Spent so far', value: `₹${getTripSpent(trip).toLocaleString('en-IN')}` },
